		...
		<table class="table table-bordered">
			...
			<tbody>
				@foreach($sale->saleitems as $saleitem)
				    <tr>
				    	<td>{{ $saleitem->product->code }}<input type="hidden" name="olditems[{{ $loop->index }}][id]" value="{{ $saleitem->id }}"></td>
				    	<td>{{ $saleitem->product->name }}</td>
				    	<td>{{ $saleitem->product->sale_price }}</td>
				    	<td><input type="number" name="olditems[{{ $loop->index }}][qty]" value="{{ $saleitem->quantity }}"></td>
				    	<td>
				    		<a data-controller="button" data-turbo="true" data-action="button#confirm" data-button-confirm="Are you sure?" class="btn btn-link" href="{{ url('/admin/sales/saleitems/delete/'. $saleitem->id) }}">
