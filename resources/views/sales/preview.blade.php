    ...
    <div class="bg-white rounded shadow-sm p-4 py-4 d-flex flex-column">
        <div class="invoice-body">
        	<div class="form-group mb-0">
        		<a href="{{ url('/admin/sales/invoice/print/'. $sale->id) }}" class="btn btn-link" style="text-decoration: none;" target="_blank"><strong><x-orchid-icon path="printer"/> Print / Download</strong></a>
        	</div>
		      	...
		      	<tbody>
		      		@foreach($sale->saleitems as $saleitem)
		      			<tr>
		        			...
		        			<td class="desc-col"><h3>{{ $saleitem->product->code }} [{{ $saleitem->product->name }}]</h3></td>
		        			<td>{{ $saleitem->product->sale_price }} MMK</td>
		        			<td>{{ $saleitem->quantity }}</td>
		        			<td>{{ $saleitem->product->sale_price * $saleitem->quantity }} MMK</td>
		        		</tr>
		      		@endforeach
		      	</tbody>
		      	<tfoot>
